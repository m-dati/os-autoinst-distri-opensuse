...
sub run {
    ...
    while (time() - $time0 <= $timeout) {
        ...
        if ($status =~ /no result returned/ig) {
            sleep(60);
            record_info("AITL WARN:", "no results:\n" . $status);
            next;
        }
        ...
        # Print the status
        diag("Unfinished AITL Jobs! Running:", Dumper($status_data));

        # Wait before checking again
        sleep(65);
    }

    # No die: let the test executed be considered:
    record_info("TIMEOUT", "Tests incomplete after $timeout sec.", result => 'fail') if (time() - $time0 > $timeout);
    ...
    die "AITL tests terminated with FAILED: " . $status_data->{FAILED} . "\n" if ($status_data->{FAILED} > 0);
}
...
sub json_to_xml {
    ...
    foreach my $test (@$data) {
        ...
        if ($test->{status} =~ /FAILED/) {
            ...
            my $skipped = $dom->createElement('skipped');
            $skipped->setAttribute('message', 'stopped' . "\n" . 'incomplete');
            $testcase->appendChild($skipped);
