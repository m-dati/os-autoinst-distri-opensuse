...
sub finalize {
    ...
    my $upload;
    my $provider = $self->{run_args}->{my_provider};
    ...
    if ($self->{run_args}) {
        ...
        # check Provider
        if ($provider) {
            my $dumpable_provider = Storable::dclone($provider);
            ...
            record_info('UNDEF. provider', $local_text);
            diag($local_text);
        }
        # check Instance
        if ($instance) {
            my $dumpable_instance = Storable::dclone($instance);
            ...
            record_info('UNDEF. instance', $local_text);
            diag($local_text);
        }
    } else {
        $start_text .= 'running arguments undefined (ref.: $self->{run_args}).';
        record_info('FAILED finalize', $start_text);
...
sub _upload_logs {
    ...
    script_run("sudo chmod a+r " . $ssh_sut_log, proceed_on_failure => 1, quiet => 1);
    upload_logs($ssh_sut_log, failok => 1, log_name => $ssh_sut_log . ".txt");

    if ($self->{run_args} && $instance) {
        unless ($instance->ssh_is_ok($instance->public_ip)) {
            diag($start_text . 'unresponsive ssh port from instance/ip:' . $instance->public_ip);
            return;
        }
        ...
        for my $instance_log (@instance_logs) {
            next unless ($instance->ssh_script_run("[ -e $instance_log ]", timeout => 0, quiet => 1, proceed_on_failure => 1) == 0);
            $instance->ssh_script_run("sudo chmod a+r " . $instance_log, timeout => 0, quiet => 1, proceed_on_failure => 1);
