...
sub finalize {
    ...
    $chk = script_run("sudo ls -d /root/terraform", proceed_on_failure => 1, timeout => 0);
    ...
    $chk = script_run("sudo ls -d /root/terraform", proceed_on_failure => 1);
...
sub _upload_logs {
    ...
    if ($self->{run_args} && $self->{run_args}->{my_instance}) {
        ...
        for my $instance_log (@instance_logs) {
            my $check = $self->{run_args}->{my_instance}->ssh_script_run("sudo ls $instance_log", timeout => 0, quiet => 1, proceed_on_failure => 1);
