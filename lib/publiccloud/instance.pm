...
sub measure_boottime() {
    ...
    my $softreboot = $perf & 2;    # bit-1 on
    my $hardreboot = $perf & 4;    # bit-2 on
...
sub store_in_db() {
    ...
    my $softr = get_var('PUBLIC_CLOUD_PERF_COLLECT', 0) & 2;    # bit-1 on
    my $hardr = get_var('PUBLIC_CLOUD_PERF_COLLECT', 0) & 4;    # bit-2 on
